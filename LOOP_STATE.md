...
- [x] Named claim that `import-judgekit` CLI rejects nested non-object `--human-labels` JSON (`{"human_labels": [...]}` / `{"human_labels": "x"}`) with exit 1 and an `error:` stderr line when bare ratings require separate gold (cost: S) (touched: 2026-09-03)
  - Done: `tests/test_adapter.py::test_import_judgekit_cli_rejects_nested_non_object_human_labels_json`.
- [x] Named claim that `import-judgekit` CLI rejects empty `--human-labels` JSON (`{}` or `{"human_labels": {}}`) with exit 1 and an `error:` stderr line when bare ratings require separate gold (cost: S) (touched: 2026-09-04)
  - Done: `tests/test_adapter.py::test_import_judgekit_cli_rejects_empty_human_labels_json`.
- [x] Named claim that `import-judgekit` CLI rejects a top-level non-object panel JSON (array / scalar) with exit 1 and an `error:` stderr line (cost: S) (touched: 2026-09-05)
  - Done: `tests/test_adapter.py::test_import_judgekit_cli_rejects_non_object_panel_json`.
- [x] Named claim that `import-judgekit` CLI rejects a panel envelope whose `ratings` field is a non-object (array / scalar) with exit 1 and an `error:` stderr line (cost: S) (touched: 2026-09-06)
  - Done: `tests/test_adapter.py::test_import_judgekit_cli_rejects_non_object_ratings_json`.
- [x] Named claim that `import-judgekit` CLI rejects a panel envelope whose `judges` field is a non-object (array / scalar) with exit 1 and an `error:` stderr line carrying `'judges' must be a JSON object when present` (cost: S) (touched: 2026-09-07)
...
- [x] Named claim that `import-judgekit` CLI stderr for missing judge coverage lists the missing human-labeled item id(s) and appends `(+N more)` when more than five gold items lack ratings for `--judge` (cost: S) (touched: 2026-09-12)
  - Done: `tests/test_adapter.py::test_import_judgekit_cli_missing_judge_coverage_lists_item_ids_with_overflow`.
- [x] Named claim that `panel_to_run` library `ValueError` lists missing human-labeled item ids in sorted order with the same five-id preview and `(+N more)` overflow as the CLI claim (cost: S) (touched: 2026-09-13)
  - Done: `tests/test_adapter.py::test_panel_to_run_missing_coverage_lists_item_ids_with_overflow`.
- [x] Named claim that `panel_to_run` rejects an empty `judge_id` (`""`) with a `ValueError` carrying `judge_id is required` (cost: S) (touched: 2026-09-14)
...
- [x] Named claim that `panel_to_run` rejects an unknown `aggregate` value with a `ValueError` carrying `aggregate must be one of` (and listing the allowed set) (cost: S) (touched: 2026-09-17)
  - Done: `tests/test_adapter.py::test_panel_to_run_rejects_unknown_aggregate`.
- [x] Named claim that `import-judgekit --aggregate` argparse `choices` equal `sorted(AGGREGATES)` (and `--help` lists both `first` and `modal`) (cost: S) (touched: 2026-09-18)
...
- [x] Named claim that `panel_to_run` signature default for `aggregate` is `"modal"` (matching CLI argparse default), so library callers omitting the kwarg get the same collapse rule (cost: S) (touched: 2026-09-21)
  - Done: `tests/test_adapter.py::test_panel_to_run_aggregate_default_is_modal`.
- [x] Named claim that omitting `aggregate=` on `panel_to_run` yields the same anchor scores as `aggregate="modal"` on a panel where modal and first disagree (cost: S) (touched: 2026-09-22)
  - Done: `tests/test_adapter.py::test_panel_to_run_omit_aggregate_matches_explicit_modal`.
- [x] Named claim that omitting `--aggregate` on `import-judgekit` yields the same written `anchor_scores` as `--aggregate modal` on a panel where modal and first disagree (cost: S) (touched: 2026-09-23)
  - Done: `tests/test_adapter.py::test_import_judgekit_omit_aggregate_matches_explicit_modal`.
- [ ] Named claim that CLI omit-`--aggregate` and library omit-`aggregate=` write the same `anchor_scores` on a panel where modal and first disagree (cost: S)
...
- 2026-09-21: panel_to_run aggregate signature default modal claim - named test asserts `inspect.signature(panel_to_run).parameters["aggregate"].default == "modal"` and keyword-only; README cites the claim; pytest 100, ruff clean.
...
- 2026-09-12: import-judgekit CLI missing-coverage item-preview claim - named test asserts stderr lists missing human-labeled item ids and appends `(+N more)` after the first five when more than five gold items lack `--judge` ratings; exit 1 / `error:` / no traceback / no outputs unchanged; README cites the claim; pytest 90, ruff clean.
...
- 2026-09-05: import-judgekit CLI non-object-panel claim - named test asserts top-level array/scalar `--panel` JSON prints `error:` + `panel JSON must be an object` on stderr, exits 1, writes no outputs, and does not dump a traceback; README cites the claim; pytest 83, ruff clean.
- 2026-09-04: import-judgekit CLI empty-human-labels claim - named test asserts empty `{}` / `{"human_labels": {}}` `--human-labels` JSON (bare ratings) prints `error:` + `human_labels are empty` on stderr, exits 1, writes no outputs, and does not dump a traceback; README cites the claim; pytest 82, ruff clean.
- 2026-09-03: import-judgekit CLI nested-non-object-human-labels claim - named test asserts nested `{human_labels: array|scalar}` `--human-labels` JSON (bare ratings) prints `error:` + `'human_labels' must be an object` on stderr, exits 1, writes no outputs, and does not dump a traceback; README cites the claim; pytest 81, ruff clean.
