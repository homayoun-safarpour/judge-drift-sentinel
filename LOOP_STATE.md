...
- **Why:** Field-level non-object `judges` is locked on main; empty `ratings` remains the next fail-closed surface on the same parse path (parallel to the empty `--human-labels` claim). Gates green — HOLD on feature work until Daily ships this enrichment.
