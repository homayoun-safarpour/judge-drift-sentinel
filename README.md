...
file, malformed `--human-labels` JSON, non-object `--human-labels`
JSON (top-level array / scalar), nested non-object `--human-labels`
JSON (`{"human_labels": [...]}` / `{"human_labels": "x"}`), and empty
