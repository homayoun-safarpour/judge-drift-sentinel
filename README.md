...
An empty `judge_id` (`""`) on `panel_to_run` fails closed with a
`ValueError` carrying `judge_id is required` rather than falling through
into the missing-coverage path
